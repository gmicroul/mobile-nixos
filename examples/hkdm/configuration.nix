{
  ...
  nurpkg-DataEraserC = nur-DataEraserC.pkgs {inherit pkgs;};
in {
  imports = [
    ./hkdm.nix
    #./bluetooth.nix
  ...
  config = lib.mkMerge [
    ...
    # HKDM
    {
      ...
      services.hkdm.configFile = "${nurpkg-DataEraserC.ttyescape}/etc/hkdm/config.d/ttyescape.toml"; # Provide the config file path here
      services.hkdm.package = nurpkg-DataEraserC.hkdm;
    }
  ];
}
